...
> **About this edition.** Pebble was created by [Brian Gao](https://github.com/thebriangao/pebble) — all credit for the original engine goes to him (MIT licensed, license preserved here). This repository is the **multiplayer edition**: on top of the original 1.0.3 it adds LAN co-op that finds itself on any computer (and a twelve-character code for when it can't), standalone SMP-style dedicated servers (`pebble serve`), permanent player identities with friend codes, custom skins, visible armor and held items with a first-person viewmodel, and a game-speed slider — see the [CHANGELOG](CHANGELOG.md) for everything.
...
Sources/pebsmoke/     golden test harness (641 checks against goldens/*.json)
...
- If the engine itself seems wrong (worldgen, physics, redstone): the tail of `pebble test` — it should print `641 passed, 0 failed`.
