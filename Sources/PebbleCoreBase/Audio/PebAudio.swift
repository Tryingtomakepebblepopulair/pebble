// The synthesized audio engine: oscillator/noise voices with RBJ biquads,
// envelopes, vibrato and scheduled sub-sounds; positional stereo, underwater
// lowpass, cave feedback-delay reverb, generative music and jukebox discs.
// Pebble owns no sound FILES — every effect is synthesized here, so this
// whole engine is portable and only the output sink is per-platform
// (PORTING module 10). Extracted verbatim from the AVFoundation Audio.swift.
...
// engine
// ---------------------------------------------------------------------------
public final class PebAudioEngine {
    ...
    public var listenerX = 0.0, listenerY = 0.0, listenerZ = 0.0, listenerYaw = 0.0
    public var underwater = false
    public var caveFactor = 0.0
    public var volumes: [String: Double] = [:]
    public var onSubtitle: ((String) -> Void)?
    public var musicTimer = 600
    ...
    /// prepare the synth for a sink running at `sampleRate`. Idempotent —
    /// the platform sink calls this once it knows its own rate.
    public func start(sampleRate: Double) {
        if inited { return }
        inited = true
    ...
    public func applyVolumes(_ v: [String: Double]) {
        volumes = v
    ...
    public func play(_ name: String, _ x: Double, _ y: Double, _ z: Double, _ volume: Double = 1, _ pitch: Double = 1) {
    ...
    public func playDisc(_ discName: String, _ x: Double, _ y: Double, _ z: Double) {
        guard inited else { return }
        stopDisc()
    ...
    // ---- render ---------------------------------------------------------------
    /// mix `frames` stereo samples into two planar float buffers. Both are
    /// cleared here, so the sink can hand over uninitialised memory. Runs on
    /// whatever thread the platform sink drives.
    public func render(_ outL: UnsafeMutablePointer<Float>,
                       _ outR: UnsafeMutablePointer<Float>, _ frames: Int) {
        let n = frames
        ...
        if !voiceInbox.isEmpty {
            ...
            // Cap the scheduled set. It has to clear the biggest thing that
            // schedules up front: a 60-second jukebox disc, which lays down
            // ~600 plucks at once (a generative track is ~250). At 512 the
            // eviction below dropped the EARLIEST voices — the ones about to
            // play — so a disc stayed silent for its first ~23 seconds.
            // Future voices are skipped by the sample loop before they start,
            // so a bigger set costs nothing until those notes come due.
            if local.count > 2048 { local.removeFirst(local.count - 2048) }
        }
