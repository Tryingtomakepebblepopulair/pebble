...
macOS is unchanged: same app, same saves, same green checks — 641 of them now.
...
| `pebsmoke` / `pebsmokecore` | macOS full suite (641) / portable suite (606) — the one Windows CI runs. | macOS / all |
...
swift run -c release pebsmoke     # macOS: 641 checks (self-assigns a temp data root)
swift run pebsmokecore            # portable: 606 checks, runs anywhere
