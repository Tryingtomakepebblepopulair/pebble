...
macOS is unchanged: same app, same saves, same green checks — 632 of them now.
...
| `pebsmoke` / `pebsmokecore` | macOS full suite (632) / portable suite (597) — the one Windows CI runs. | macOS / all |
...
swift run -c release pebsmoke     # macOS: 632 checks (self-assigns a temp data root)
swift run pebsmokecore            # portable: 597 checks, runs anywhere
